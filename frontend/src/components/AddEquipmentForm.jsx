...
const AddEquipmentForm = ({ onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!user.company) {
                throw new Error('User company is not found.');
            }
            ...
            const newMachine = {
                ...
                location_lon: parseFloat(locationLon),
            };

            const response = await fetch(
                `http://localhost:3000/companies/${user.company}/machines`,
                {
                    method: 'POST',
                    headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData || 'Failed to add machine');
            }
